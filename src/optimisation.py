...
def mf_conjugate_grad(data, settings):
    ...
    a_factor = np.random.randn(n_side_info, rank)  # (n_x, r)
    b_factor = np.random.randn(n_cols, rank)  # (n_y, r)

    idx_mask = sp.sparse.find(mask)  # return the indices and values of the nonzero elements of a matrix
    ...
    a = a_factor.flatten()
    b = b_factor.flatten()
    x = np.append(a, b)  # we optimize a and b matrices

    # nonlinear conjugate gradient algorithm

    x = fmin_cg(mf_obj_wrapper, x, mf_grad_wrapper,  # obj, starting x0, grad_obj
                args=(data, settings, temp_bucket),  # arg of the f and f_prime
                disp=True,  # display convergence message
                retall=False,
                full_output=False,
                maxiter=10000,
                gtol=settings.opt_tol)  # Stop when the norm of the gradient is less than gtol

    # get a and b after optimization
    a_factor = np.reshape(x[:n_side_info * rank], (n_side_info, rank))
    b_factor = np.reshape(x[n_side_info * rank:], (n_cols, rank))

    w_matrix = data.side_info @ (a_factor @ b_factor.T) # G = A(B*) ==>
...
def mf_obj(a_factor, b_factor, y_matrix, mask, idx_mask, param1, temp_bucket):
    ...
    if np.all(a_prev == a_factor) and np.all(b_prev == b_factor): # pas de changement. On ne recalcule pas la loss
    ...
    obj = 0.5 * norm_sparse(loss_part, 'fro') ** 2 + 0.5 * param1 * (
            norm(a_factor, 'fro') ** 2 + norm(b_factor, 'fro') ** 2)
...
def loss_part_fun(a_factor, b_factor, y_matrix, mask, idx_mask):
    ...
    w_matrix = csr_matrix((w_bar_vec, (idx_mask[0], idx_mask[1])), shape=(n_rows, n_cols)) # G matrix (AB*)
    loss_part = y_matrix - w_matrix   # phi(yi) - AN*(xi)
